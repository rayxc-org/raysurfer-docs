...
When serving a request for a specific customer, just pass their workspace ID:
...
The same works with `RaysurferClient`:
...
# Just pass the workspace ID
async with RaysurferClient(options, workspace_id="ws_abc123...") as client:
...
|--------------|--------------|----------|
| No workspace | Default org namespace | Team sharing snippets |
| `workspace_id` | **Per-workspace isolation** | **Multi-tenant SaaS** |
...
Without `workspace_id`, the SDK uses your organization's shared namespace. All
users in your org share the same cache — fine for internal use, but not for

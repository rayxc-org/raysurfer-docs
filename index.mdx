...
<Note>
Raysurfer's value kicks in when you ask a query **similar** to a previous one—not just identical. The semantic matching finds related patterns, so "fetch GitHub repos" and "get trending GitHub projects" can both hit the same cached code.
</Note>

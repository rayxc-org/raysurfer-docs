...
When your agent runs a task similar to one it's done before, Raysurfer returns the cached result instantly instead of regenerating it.

```
First run:  Your query → Code generated → Result cached
Second run: Similar query → Cache hit → Code retrieved (instant)
```

Queries don't need to be identical—Raysurfer uses semantic matching to find related patterns. "Fetch user data from the API" and "Get customer info from the endpoint" can hit the same cache.
...
Raysurfer maintains a curated collection of public snippets sourced from popular open-source GitHub repositories. These are validated code patterns that any user can include in their retrieval results.
...
Public snippets are read-only — they cannot be edited or deleted by users. They are curated by the Raysurfer team and ranked alongside your private snippets using the same scoring system.

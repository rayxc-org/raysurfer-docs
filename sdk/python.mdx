...
async with AsyncRaySurfer(api_key="your_api_key") as client:
    ...
    # 3. Upload a new code file after execution (voting triggered by default)
    file = FileWritten(path="fetch_repos.py", content="def fetch(): ...")
...
| `upload_new_code_snip(task, file_written, succeeded, use_raysurfer_ai_voting, user_vote, execution_logs)` | Store a single code file (AI votes by default, or provide your own vote) |

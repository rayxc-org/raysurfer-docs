...
| `upload(task, file_written, succeeded, ...)` | Store a code file for future reuse |
...
Also available as an integration guide: [Register Python Functions](/integrations/register-python-functions).

Register local tools, then either pass `user_code` (primary mode) or use optional sandbox codegen with your own key + prompt:
...
The `@rs.tool` decorator can wrap any Python function you want to expose as a tool.

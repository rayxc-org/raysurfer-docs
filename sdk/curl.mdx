...
| `auto_vote` | boolean | `true` | Auto-vote on each stored block |
...
<Note>
  Voting happens automatically when you use `auto_vote: true` in the store endpoints. Use the vote endpoint directly only when you need manual control — for example, when code was retrieved from cache and you want to report whether it worked.
</Note>

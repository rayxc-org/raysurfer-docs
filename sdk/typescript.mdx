...
Each `CodeFile` contains `filename`, `source`, `description`, `verdictScore`, `thumbsUp`, `thumbsDown`, and `similarityScore`.
...
// 1. Get cached code files (includes addToLlmPrompt)
const result = await client.getCodeFiles({ task: "Fetch GitHub trending repos" });

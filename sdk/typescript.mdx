...
Each `CodeFile` contains `codeBlockId`, `filename`, `source`, `description`,
`verdictScore`, `thumbsUp`, `thumbsDown`, and `similarityScore`.

### Method Reference

| Method | Description |
|--------|-------------|
| `search({ task, topK?, minVerdictScore?, preferComplete? })` | Unified search for cached code (recommended) |
...
| `uploadNewCodeSnip(task, fileWritten, succeeded, cachedCodeBlocks?, autoVote?, executionLogs?)` | Store a single code file for future reuse (auto-votes by default) |
| `uploadBulkCodeSnips(prompts, filesWritten, logFiles?, autoVote?)` | Bulk upload prompts/logs/code for sandboxed grading (votes each file) |

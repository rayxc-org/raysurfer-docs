...
<Note>
  Voting is triggered automatically by `upload_new_code_snip` / `uploadNewCodeSnip` when `auto_vote` is `true` (the default), and also by the high-level `RaysurferClient`. Use `vote_code_snip` / `voteCodeSnip` directly only when you need manual voting control.
</Note>

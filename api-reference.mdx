...
**Python** — `RaysurferClient(options, public_snips=False, snips_desired=None, namespace=None, debug=False)`:

| Parameter | Type | Description |
|-----------|------|-------------|
| `options` | `ClaudeAgentOptions` | Standard Claude Agent SDK options |
| `public_snips` | `boolean` | Include public/shared snippets (default: `false`) |
| `snips_desired` | `"company"` \| `"client"` | Scope of private snippets |
...
| `publicSnips` | `boolean` | Include public/shared snippets (default: `false`) |
| `snipsDesired` | `"company"` \| `"client"` | Scope of private snippets |
| `namespace` | `string` | Custom namespace for storage/retrieval |
| `debug` | `boolean` | Enable debug logging |
...
| `files_written` / `filesWritten` | `FileWritten[]` | required | Files created during execution |
| `succeeded` | `boolean` | required | Whether the task completed successfully |
| `auto_vote` / `autoVote` | `boolean` | `true` | Auto-vote on newly stored blocks |
| `execution_logs` / `executionLogs` | `string \| null` | `null` | Captured stdout/stderr for vote context |

<Note>
  When `auto_vote` is `true` (the default), each newly stored code block automatically receives a thumbs-up vote if the task succeeded. This provides an initial quality signal without waiting for the code to be reused and re-evaluated.
</Note>
...
<Note>
  Voting is triggered automatically by `upload_new_code_snips` / `uploadNewCodeSnips` when `auto_vote` is `true` (the default), and also by the high-level `RaysurferClient`. Use `vote_code_snip` / `voteCodeSnip` directly only when you need manual voting control.
</Note>

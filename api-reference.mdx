...
<Tip>
  **LLM prompt augmentation**: The convenience method `get_code_files()` / `getCodeFiles()` wraps `search()` and returns an `add_to_llm_prompt` string you can append directly to your system prompt. It's the same search results, just pre-formatted for LLM injection.
</Tip>
...
<Note>
  Voting is triggered automatically by `upload_new_code_snip` when `use_raysurfer_ai_voting` is `true` (the default), and also by the high-level `RaysurferClient`. Use `vote_code_snip` directly only for voting on cached blocks after reuse.
</Note>
...
|-------|------|-------------|
| `matches` | [SearchMatch](#searchmatch)`[]` | Matching code blocks with full scoring |
...
Returned by [`upload_new_code_snip()`](#upload_new_code_snip--uploadnewcodesnip).
...
A single result from [`search()`](#search). Contains a full [CodeBlock](#codeblock) plus scoring metadata.

| Field | Type | Description |
|-------|------|-------------|
| `code_block` / `codeBlock` | [CodeBlock](#codeblock) | The matched code block with full source and metadata |
| `combined_score` / `combinedScore` | `float` | Overall relevance score (0–1). Formula: `0.6 * similarity + 0.25 * verdict + 0.15 * confidence + resilience` |
| `vector_score` / `vectorScore` | `float` | Semantic similarity between your task and the code block (0–1) |
| `verdict_score` / `verdictScore` | `float` | Community quality rating: `thumbs_up / (thumbs_up + thumbs_down)`, defaults to `0.3` if unrated |
| `error_resilience` / `errorResilience` | `float` | Score for how well the code handles errors (0–1) |
...
| `input_schema` / `inputSchema` | `dict` / `Record<string, unknown>` | JSON schema describing expected inputs |
| `output_schema` / `outputSchema` | `dict` / `Record<string, unknown>` | JSON schema describing outputs |
...
A file created during agent execution. Used by [`upload_new_code_snip()`](#upload_new_code_snip--uploadnewcodesnip) and [`upload_bulk_code_snips()`](#upload_bulk_code_snips--uploadbulkcodesnips).

...
async with RaysurferClient(options) as client:
    await client.raysurfer_query("Your task here")
...
for await (const msg of client.raysurferQuery("Your task here")) {
  console.log(msg);
}
```
...
| Method | Description |
|--------|-------------|
| `raysurfer_query(prompt)` / `raysurferQuery(prompt)` | Send a query with automatic cache retrieval |
| `raysurfer_response()` | Async generator yielding response messages (Python only) |
...
| `cache_dir` / `cacheDir` | `string` | `undefined` | Directory where files will be written. If provided, `add_to_llm_prompt` includes full paths. |
